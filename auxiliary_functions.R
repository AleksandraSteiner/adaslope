...
initialization_functions = function(X, y, 
                                    beta.start, 
                                    sigma.known, 
                                    sigma.init, 
                                    lambda, 
  ...
  list(beta, gamma, sigma, theta, c, Big_Sigma, rk)
}
...
print_iteration_status = function(print_iter, t, beta, sigma, cstop) {
...
generate_X = function(Big_Sigma, mu, beta, sigma, y, X, X_mis) {
  ...
  if(missingcols != 0) {
    ...
    if(scale) scale_X(X_mis, scale, row.w, std.w, mean.w)
    X_mis
  }
  X
}
...
update_estimations_cache = function(beta, sigma, gamma, theta, c, 
                                    estimations_cache) {
...
estimate_sigma_ML = function(X, y, beta, lambda, 
                              sigma.known, sigma) {
  if(is.na(sigma.known)){
    p = ncol(X)
    ...
    (sqrt(sum_lamwbeta ^ 2 + 4 * n * RSS) + sum_lamwbeta) / (2 * n)
  }
  else
    sigma
}
...
iterate_algorithm = function(t = 0, 
  ...
  while(cstop > tol_em & t < maxit | t < 20) { #why exactly this condition
    t = t + 1
    print_iteration_status(print_iter, t, beta, sigma, cstop)
    ...
    X = generate_X(Big_Sigma, mu, beta, sigma, y, X, X_mis)
    ...
    sigma = estimate_sigma_ML(X, y, beta, lambda, 
                               sigma.known, sigma)
